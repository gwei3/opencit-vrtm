...
bool ReadRSAKeyfromFile(const char* szKeyFile, RSAKeyPair *rsaKeyPairOut)
{
    ...
    PrintBytes("P:", (u8*)pRSAKey->m_pbnP->m_pValue, pRSAKey->m_iByteSizeP);
    PrintBytes("Q:", (u8*)pRSAKey->m_pbnQ->m_pValue, pRSAKey->m_iByteSizeQ);
    PrintBytes("M:", (u8*)pRSAKey->m_pbnM->m_pValue, pRSAKey->m_iByteSizeM);
    PrintBytes("E:", (u8*)pRSAKey->m_pbnE->m_pValue, pRSAKey->m_iByteSizeE);
    PrintBytes("D:", (u8*)pRSAKey->m_pbnD->m_pValue, pRSAKey->m_iByteSizeD);
...
int doTrustOpX(int opcode, int inSize, byte* inData, int* poutSize, byte* outData) {
	...
	case TCSERVICEATTESTFORFROMAPP:
            size = encodeTCSERVICEATTESTFORFROMAPP(inSize, inData, PARAMSIZE - size, &rgBuf[size]);
             break;
	case TCSERVICESEALFORFROMAPP:
            size = encodeTCSERVICESEALFORFROMAPP(inSize, inData, PARAMSIZE - size, &rgBuf[size]);
            break;
	case TCSERVICEUNSEALFORFROMAPP:
	    PrintBytes(" Input param checking before encode", (byte*)inData, inSize);
            size = encodeTCSERVICEUNSEALFORFROMAPP(inSize, inData, PARAMSIZE - size, &rgBuf[size]);
            break;
	default:
            size= -1;
            break;
       ...
       if(size < 0) {
            fprintf(stdout, "getQuote: encodeTCSERVICEATTESTFORFROMAPP failed\n");
            close (fd1);
            return -11;
       }
       pReq->m_procid= 0;
...
again:
       ...
       switch (opcode){
            case TCSERVICEATTESTFORFROMAPP:
                    rc = decodeTCSERVICEATTESTFORFROMTCSERVICE(poutSize, outData, &rgBuf[sizeof(tcBuffer)]);
                    break;
            case TCSERVICESEALFORFROMAPP:
                    rc = decodeTCSERVICESEALFORFROMTCSERVICE(poutSize, outData, &rgBuf[sizeof(tcBuffer)]);
                    break;
            case TCSERVICEUNSEALFORFROMAPP:
                    rc = decodeTCSERVICEUNSEALFORFROMTCSERVICE(poutSize, outData, &rgBuf[sizeof(tcBuffer)]);
                    break;
	    default:
                rc= 0;
                break;
       ...
       if(!rc) {
           fprintf(stdout, "getQuote: gettcBuf for decodeTCSERVICEATTESTFORFROMTCSERVICEfailed\n");
           close (fd1);
           return -12;
       }
       size = *poutSize;
...
int Seal(Data_buffer * secretIn, Data_buffer *dataOut)
{
	...
	iRet= doTrustOp(TCSERVICESEALFORFROMAPP, &uType, secretIn->len, secretIn->data,
...
int UnSeal(Data_buffer *dataIn, Data_buffer *secretOut)
{
	...
	iRet= doTrustOp(TCSERVICEUNSEALFORFROMAPP, &uType, dataIn->len, dataIn->data,
...
int Attest(Data_buffer *dataIn, Data_buffer *dataOut)
{
	...
	iRet= doTrustOp(TCSERVICEATTESTFORFROMAPP, &uType, dataIn->len, dataIn->data,
                        (int *)&dataOut->len, dataOut->data);
	return iRet;
}
