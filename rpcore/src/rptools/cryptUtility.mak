RPROOT=../..
E=          $(RPROOT)/bin/release
B=          $(RPROOT)/build/release/cryptUtilityobjects
LIB=        $(RPROOT)/lib
INC=        $(RPROOT)/inc
...
#CFLAGS=	    -D TEST -DFILECLIENT -DTEST1 -D NOAESNI $(DEBUG_CFLAGS) -D TEST -DTEST1 -D QUOTE2_DEFINED -D NEWANDREORGANIZED SUJAN - REMOVE THE TEST1
CFLAGS=	    -D TEST -DFILECLIENT  -D NOAESNI $(DEBUG_CFLAGS)  -D QUOTE2_DEFINED -D NEWANDREORGANIZED
...
dobjs=      $(B)/cryptUtility.o $(B)/logging.o $(B)/jlmcrypto.o $(B)/aes.o \
	    ...
	    $(B)/validateEvidence.o $(B)/cert.o $(B)/quote.o \
	    \
	    $(B)/cryptoHelper.o $(B)/fastArith.o $(B)/mpBasicArith.o $(B)/mpModArith.o \
	    $(B)/mpNumTheory.o $(B)/fileHash.o $(B)/encryptedblockIO.o \
	    $(B)/tinystr.o $(B)/tinyxmlerror.o $(B)/tinyxml.o $(B)/tinyxmlparser.o 

#$(B)/resource.o $(B)/vault.o $(B)/signedAssertion.o $(B)/accessControl.o 

all: $(E)/cryptUtility

$(E)/cryptUtility: $(dobjs)
	@echo "cryptUtility"
	$(LINK) -o $(E)/cryptUtility $(dobjs) -lpthread
