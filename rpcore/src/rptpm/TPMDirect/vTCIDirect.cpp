...
//
bool TPMpcrExtend(int fd, int pcr, byte * indigest, int* psize, byte* out)
{
    byte    cmdblob[TPMMAXBUF];
    int     cmdoffset= 0;
    byte    ansblob[TPMMAXBUF];
    ...
    // check parameters
    if(*psize<20) {
        fprintf(g_logFile, "TPMpcrExtend buffer too small %d\n", *psize);
        return false;
    }
    ...
    Load(true, cmdblob, &cmdoffset, (byte*)&cmdin, sizeof(u32));
    Load(true, cmdblob, &cmdoffset, (byte*)&pcr, sizeof(u32));
    Load(true, cmdblob, &cmdoffset, indigest, 20);

#ifdef TPMTEST
    PrintBytes("TPMpcrExtendcommand: ", cmdblob, cmdoffset);
#endif
...
#ifdef TPMTEST
    PrintBytes("TPMpcrExtend response: ", ansblob, outsize);
#endif
...
bool tpmStatus::extendPCRValue(int pcr, byte* indigest, int *psize, byte* out)
{
  //return true;
  return TPMpcrExtend(m_tpmfd, pcr, indigest, psize, out);
}
                          ...
                          unsigned* pquotedSize, byte* quotedData, byte *locality)
{
...
#ifdef TPMTEST_DOESNOTCOMPILE_SUJAN
...
#ifdef TPMTEST
    fprintf(g_logFile, "size of pcrselection: %d\n", sizeof(pcrselect));
    PrintBytes("pcrselect: ", (byte*)&pcrselect, sizeof(pcrselect));
    fprintf(g_logFile, "handle from osap: %08x\n", hauth);
    PrintBytes("shared secret: ", shared_secret, 20);
    PrintBytes("encauth: ", encauth, 20);
    PrintBytes("digest: ", digest, 20);
	...
	*locality = pcrinfo.m_locatrelease;
finish:
#ifdef TERMINATEHANDLES
    if(hauth!=0)
        TPMterminatehandle(m_tpmfd, hauth);
                            ...
                            u32 sizeversion, byte* versionInfo)
{
	...
	rsakey.m_pbnM = new bnum(32);
    rsakey.m_pbnE = new bnum(32);
 
	memcpy(rsakey.m_pbnM->m_pValue, m_rgaikmodulus, m_iaikmodulusLen);
	memcpy(rsakey.m_pbnE->m_pValue, (byte*)bnE.m_pValue, bnE.mpSize());
	
	RSAEncrypt_i(rsakey, dataSize, (byte*) signedData, &len, result);
