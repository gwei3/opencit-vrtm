...
#ifdef TEST
	fprintf(stdout, "tcsd hashprep.cpp, calling getCompositePCRHash next\n");
#endif
	if(!g_oTpm.getCompositePCRHash( &sizeOfComposite, composite))
		return false;
		
	*pcrs = sizeOfComposite;
	memcpy(pcrDigest, composite, sizeOfComposite);

#ifdef TEST
	fprintf(stdout, "getCompositePCRHash done, sizeOfComposite = %d\n", sizeOfComposite);
    PrintBytes("PcrComposite buffer hash\n", pcrDigest, sizeOfComposite);
#endif
...
{
	#ifdef TEST
	fprintf(stdout, "tcsd hashprep.cpp, calling computeTPM12compositepcrDigest next\n");
#endif
