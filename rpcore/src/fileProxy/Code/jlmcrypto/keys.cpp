...
bool RSAKey::getDataFromRoot(TiXmlElement*  pRootElement)
{
    ...
    if(strcmp(szKeyType,"RSAKeyType")==0) {
        ...
        m_iByteSizeM= 0;
        if(szRsaKeyM) {
            if(!fromBase64(strlen(szRsaKeyM), szRsaKeyM, &iOutLen, tbuf)) {
                fprintf(g_logFile, "Cant base64 decode M in RSA key\n");
                return false;
            }
            m_iByteSizeM= iOutLen;
            revmemcpy(m_rgbM, tbuf, iOutLen);
        }
        ...
        m_iByteSizeP= 0;
        if(szRsaKeyP) {
            if(!fromBase64(strlen(szRsaKeyP), szRsaKeyP, &iOutLen, tbuf)) {
                fprintf(g_logFile, "Cant base64 decode P in RSA key\n");
                return false;
            }
            m_iByteSizeP= iOutLen;
            
            revmemcpy(m_rgbP, tbuf, iOutLen);
        }

        m_iByteSizeQ= 0;
        iOutLen= BIGKEYSIZE;
        if(szRsaKeyQ) {
            if(!fromBase64(strlen(szRsaKeyQ), szRsaKeyQ, &iOutLen, tbuf)) {
                fprintf(g_logFile, "Cant base64 decode Q in RSA key\n");
                return false;
            }
            m_iByteSizeQ= iOutLen;
            revmemcpy(m_rgbQ, tbuf, iOutLen);
        }

        iOutLen= BIGKEYSIZE;
        m_iByteSizeE= 0;
        if(szRsaKeyE) {
            if(!fromBase64(strlen(szRsaKeyE), szRsaKeyE, &iOutLen, tbuf)) {
                fprintf(g_logFile, "Cant base64 decode E in RSA key\n");
                return false;
            }
            m_iByteSizeE= iOutLen;
            revmemcpy(m_rgbE, tbuf, iOutLen);
        }

        iOutLen= BIGKEYSIZE;
        m_iByteSizeD= 0;
        if(szRsaKeyD) {
            if(!fromBase64(strlen(szRsaKeyD), szRsaKeyD, &iOutLen, tbuf)) {
                fprintf(g_logFile, "Cant base64 decode D in RSA key\n");
                return false;
            }
            m_iByteSizeD= iOutLen;
            revmemcpy(m_rgbD, tbuf, iOutLen);
        }

        iOutLen= BIGKEYSIZE;
        m_iByteSizeDP= 0;
        if(szRsaKeyDP) {
            if(!fromBase64(strlen(szRsaKeyDP), szRsaKeyDP, &iOutLen, tbuf)) {
                fprintf(g_logFile, "Cant base64 decode DP in RSA key\n");
                return false;
            }
            m_iByteSizeDP= iOutLen;
            revmemcpy(m_rgbDP, tbuf, iOutLen);
        }

        iOutLen= BIGKEYSIZE;
        m_iByteSizeDQ= 0;
        if(szRsaKeyDQ) {
            if(!fromBase64(strlen(szRsaKeyDQ), szRsaKeyDQ, &iOutLen, tbuf)) {
                fprintf(g_logFile, "Cant base64 decode DQ in RSA key\n");
                return false;
            }
            m_iByteSizeDQ= iOutLen;
            revmemcpy(m_rgbDQ, tbuf, iOutLen);
        }
    }
    else {
        fprintf(g_logFile, "Unknown key type\n");
...
const char* szlocalKeyInfoHeader="<ds:KeyInfo xmlns:ds=\"http://www.w3.org/2000/09/xmldsig#\" KeyName='%s'>\n";
...
void kpb(const char* szMsg, byte* pbData, int iSize)
{
    printf("%s", szMsg);
    for (int i= 0; i<iSize; i++) {printf("%02x", pbData[i]);}
    printf("\n");
}
...
char*   RSAKey::SerializetoString()
{
    ...
    if(m_iByteSizeM>0) {
        iOutLen= MAXBASE64;
        revmemcpy(tbuf, (byte*)m_pbnM->m_pValue, m_iByteSizeM);
        kpb("M ", (byte*)m_pbnM->m_pValue, m_iByteSizeM);
        kpb("M ", tbuf, m_iByteSizeM);
        if(!toBase64(m_iByteSizeM, tbuf, &iOutLen, szBase64)) 
            return NULL;
        szBase64[iOutLen]= 0;
    ...
    if(m_iByteSizeE>0) {
        iOutLen= MAXBASE64;
        revmemcpy(tbuf, (byte*)m_pbnE->m_pValue, m_iByteSizeE);
        kpb("E ", tbuf, m_iByteSizeE);
        if(!toBase64(m_iByteSizeE, tbuf, &iOutLen, szBase64)) 
            return NULL;
        szBase64[iOutLen]= 0;
    ...
    if(m_iByteSizeD>0) {
        iOutLen= MAXBASE64;
        revmemcpy(tbuf, (byte*)m_pbnD->m_pValue, m_iByteSizeD);
        kpb("D ", (byte*)m_pbnD->m_pValue, m_iByteSizeD);
        kpb("D ", tbuf, m_iByteSizeD);
        if(!toBase64(m_iByteSizeD, tbuf, &iOutLen, szBase64)) 
            return NULL;
        szBase64[iOutLen]= 0;
    ...
    if(m_iByteSizeP>0) {
        iOutLen= MAXBASE64;
        revmemcpy(tbuf, (byte*)m_pbnP->m_pValue, m_iByteSizeP);
        if(!toBase64(m_iByteSizeP, tbuf, &iOutLen, szBase64)) 
            return NULL;
        szBase64[iOutLen]= 0;
    ...
    if(m_iByteSizeQ>0) {
        iOutLen= MAXBASE64;
        revmemcpy(tbuf, (byte*)m_pbnQ->m_pValue, m_iByteSizeQ);
        if(!toBase64(m_iByteSizeQ, tbuf, &iOutLen, szBase64)) 
            return NULL;
        szBase64[iOutLen]= 0;
    ...
    if(m_iByteSizeDP>0) {
        iOutLen= MAXBASE64;
        revmemcpy(tbuf, (byte*)m_pbnDP->m_pValue, m_iByteSizeDP);
        if(!toBase64(m_iByteSizeDP, tbuf, &iOutLen, szBase64)) 
            return NULL;
        szBase64[iOutLen]= 0;
    ...
    if(m_iByteSizeDQ>0) {
        iOutLen= MAXBASE64;
        revmemcpy(tbuf, (byte*)m_pbnDQ->m_pValue, m_iByteSizeDQ);
        if(!toBase64(m_iByteSizeDQ, tbuf, &iOutLen, szBase64)) 
            return NULL;
        szBase64[iOutLen]= 0;
