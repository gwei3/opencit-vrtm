...
{
    memset(rguipad, 0, SHA256_BLOCKSIZE_BYTES);
    memset(rguopad, 0, SHA256_BLOCKSIZE_BYTES);
    memset(rguNewKey, 0, SHA256_BLOCKSIZE_BYTES);
}
...
void  hmacsha256::Init(byte* rguKey, int iKeyLen) 
{
    ...
    if(k>SHA256_BLOCKSIZE_BYTES) {
     ...
     memset(rguipad + k ,0x36, SHA256_BLOCKSIZE_BYTES - k);
     memset(rguopad + k ,0x5c, SHA256_BLOCKSIZE_BYTES - k);
    ...
    // start inner hash
    oHash.Init();
    oHash.Update(rguipad, SHA256_BLOCKSIZE_BYTES);
...
void  hmacsha256::Final(byte* rguDigest) 
{
    ...
    // Outer hash
    oHash.Init();
    oHash.Update(rguopad, SHA256_BLOCKSIZE_BYTES);
