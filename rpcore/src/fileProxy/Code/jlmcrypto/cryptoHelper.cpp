                 ...
                 byte* out, bool fFast)
{
    ...
    mpZeroNum(bnMsg);
    mpZeroNum(bnOut);
	int insize = (sizein > key.m_iByteSizeM )? key.m_iByteSizeM : sizein;
                                 
    revmemcpy((byte*)bnMsg.m_pValue, in, insize /*key.m_iByteSizeM*/);
...
bool  RSAEncrypt_i(RSAKey& key, int sizein, byte* in, int* psizeout, byte* out)
{
	...
	fprintf(stdout, "%lx %lx\n" , (void*)key.m_pbnE, (void*)key.m_pbnM);
    PrintBytes((char*)"RSAEncrypt in: ", in, sizein);   
    PrintBytes((char*)"RSAEncrypt M: ", key.m_rgbM, key.m_iByteSizeM);
    PrintBytes((char*)"RSAEncrypt E: ",  key.m_rgbE, key.m_iByteSizeE);    
#endif
...
#endif
    if(!RSADecrypt_i(key, key.m_iByteSizeM, padded, psizeout, out))
        return false;
...
bool  RSAVerify(RSAKey& key, int hashType, byte* hash, byte* in)
{
    ...
    if(!RSAEncrypt_i(key, key.m_iByteSizeM, in, &size, padded)) {
              ...
              int* psizeout, byte* out)
{
    ...
    switch(keyuse) {
      default:
      case USEPUBLIC:
        if(!RSAEncrypt_i(key, key.m_iByteSizeM, padded, psizeout, out)) {
            fprintf(g_logFile, "RSASeal: encryption failed\n");
            return false;
        }
        break;
      case USEPRIVATE:
        if(!RSADecrypt_i(key, key.m_iByteSizeM, padded, psizeout, out, false)) {
            fprintf(g_logFile, "RSASeal: encryption failed\n");
            return false;
        }
        break;
      case USEPRIVATEFAST:
        if(!RSADecrypt_i(key, key.m_iByteSizeM, padded, psizeout, out, true)) {
            fprintf(g_logFile, "RSASeal: encryption failed\n");
            return false;
        }
        break;
                ...
                int* psizeout, byte* out)
{
    ...
    switch(keyuse) {
      ...
      case USEPUBLIC:
        ...
        break;
      case USEPRIVATE:
        if(!RSADecrypt_i(key, key.m_iByteSizeM, in, &size, padded, false)) {
            fprintf(g_logFile, "RSASeal: encryption failed\n");
            return false;
        }
        break;
      case USEPRIVATEFAST:
        if(!RSADecrypt_i(key, key.m_iByteSizeM, in, &size, padded, true)) {
            fprintf(g_logFile, "RSASeal: encryption failed\n");
            return false;
        }
        break;
