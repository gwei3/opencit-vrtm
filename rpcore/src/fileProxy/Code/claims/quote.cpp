...
"<Quote format='xml'>\n"\
"    %s\n" \
"    <CodeDigest alg='%s' locality='%s'> %s </CodeDigest>\n" \
"%s\n" \
"        <QuoteValue>\n" \
"%s\n" \
"        </QuoteValue>\n" \
...
//HL: add pcrSelect below
const char*   g_szQuoteTemplate= 
"<Quote format='xml'>\n"\
"    %s\n" \
"    <CodeDigest alg='%s' locality='%s' pcrSelect='%s'> %s </CodeDigest>\n" \
"%s\n" \
"        <QuoteValue>\n" \
"%s\n" \
"        </QuoteValue>\n" \
 ...
 * 
 */
//HL: above format obselete

//HL: we edited the quote format, instead of what listed above, it shall be as follows:
/*
 * 
 *  Typical quote for public key
 * 
 *  <Quote format='xml'>
 *      <CodeDigest alg='SHA256' location='0 pcrSelect="PCRindex1,PCRindex2,...,PCRindexN"> </CodeDigest>
 *      <ds:QuotedInfo>
 * 			<nonce> </nonce>\n"
 *           <ds:KeyInfo xmlns:ds="http://www.w3.org/2000/09/xmldsig#" KeyName="//www...">
 *             <KeyType>RSAKeyType</KeyType>
...
bool  Quote::init(const char* szXMLQuote)
{
    ...
    szA= ((TiXmlElement*) m_pNodeCodeDigest)->Attribute ("locality");
    if(szA==NULL) {
        fprintf(g_logFile, "Quote::init: No CodeDigest locality\n");
        return false;
    }
    m_szlocality= strdup(szA);
...
char* Quote::getQuoteLocality()
{
    if(m_szlocality==NULL)
        return NULL;
    // Fix
    return strdup(m_szlocality);
}


// ------------------------------------------------------------------
//NOTE:In current implementation of checkXMLQuote, we define PCR selected by compile time macros.
//In practice, it shall get the PCR indices from the quote it receives, see definition of g_szCertTemplate. 
bool checkXMLQuote(const char* szQuoteAlg, const char* szCanonicalQuotedBody, const char* sznonce, 
                const char* szdigest, KeyInfo* pKeyInfo, const char* szQuoteValue, const char* szlocality)
{
    Sha1    oSha1Hash;
    Sha256  oSha256Hash;
...
#ifdef TEST
    PrintBytes((char*)"Quote before reverse: ", quoteValue, outLen);
    printf("outLen =  %d ", outLen);
    //printf("0= %02x ",quoteValue[0]);
    //printf("outLen-1= %02x\n",quoteValue[outLen-1]);
#endif

    //byte qValTemp[256];
    //memcpy(qValTemp, quoteValue, outLen);
    //revmemcpy(quoteValue, qValTemp, outLen);

#ifdef TPMKSS
    unsigned char qCh;
    for (int i = 0; i < outLen/2; i++) {
		qCh= quoteValue[outLen - i -1];
		quoteValue[outLen - i -1] = quoteValue[i];	
		quoteValue[i]= qCh;       
    }
#endif
    ...
    //printf("0= %02x\n",quoteValue[0]);
    //printf("outLen-1= %02x\n",quoteValue[outLen-1]);
    PrintBytes((char*)"Quote After reverse: ", quoteValue, outLen);
#endif
...
//HL: extending to PCR17,18,19
//In practice, the PCR indices shall be read from the quote received, see definition of g_szCertTemplate. 
#ifdef PCR19 && PCR18
...
//HL: Above format out of date. 
//See definition of g_szQuoteTemplate above for the updated format.
char* encodeXMLQuote(int sizenonce, byte* nonce, int sizeCodeDigest, 
                     byte* codeDigest, const char* szlocality, 
                     const char* pcrSelection,
                     const char* szQuotedInfo, const char* szKeyInfo, 
                     int sizeQuote, byte* quote)
{
    ...
    sprintf(szB, g_szQuoteTemplate, szNonce, szdigestAlg, szlocality, 
			pcrSelection,
			szCodeDigest, szQuotedInfo, szquoteValue, szKeyInfo);
    szQuote= strdup(szB);
...
bool decodeXMLQuote(const char* szXMLQuote, char** pszAlg, char** psznonce, 
                    ...
                    char** pszquoteKeyInfo, char** pszquotedKeyInfo, char** pszlocality)
{
    Quote   oQuote;
	...
	*pszlocality = oQuote.getQuoteLocality();
    return true;
}
