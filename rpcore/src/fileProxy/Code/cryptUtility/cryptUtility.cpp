...
bool VerifyQuote(const char* szQuoteFile, const char* szCertFile)
{
	...
	char* szLocality = oQuote.getQuoteLocality();
...
#if ONERSA
	//Use decrypt operation to encrypt with private key
	if(! RSADecrypt_i(*pKey, pKey->m_iByteSizeM, rgToSign, &sizeAttested, attest)){
...
#if ONERSA
	int tmp = 0;
	if(! RSAEncrypt_i(*pKey, pKey->m_iByteSizeM, attest, &tmp, rgdecrypted)){
        fprintf(g_logFile, "mpRSAENC returned false\n");
        return false;
    }
