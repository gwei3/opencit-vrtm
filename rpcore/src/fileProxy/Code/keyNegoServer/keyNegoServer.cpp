            ...
            char** pszNotAfter, char** pszSubjName, char** pszSubjKeyID)
{
    ...
    *pszIssuerName= strdup("www.rpkss.com");
    *pszIssuerID= strdup("www.rpkss.com");
...
#endif
    // www.manferdelli.com/Herstein/Keys/fileClientProgram"
 //commented by v: for testing
 //   if(szKeyName==NULL) {
        *pszSubjName= strdup("//www.rpkss.com/Programs/Unknown");
...
bool validateQuote(const char* szPolicyKeyId, const char* szXMLQuote, 
                             const char* szEvidence) {
    ...
    if(!decodeXMLQuote(szXMLQuote, &szAlg, &szNonce, &szDigest, &szQuotedInfo,
                       &szQuoteValue, &szquoteKeyInfo, &szquotedKeyInfo, &szLocality)) {
    ...
    if(!checkXMLQuote(szAlg, szQuotedInfo, szNonce,
                      szDigest, pQuoteKey, szQuoteValue, szLocality)) {
...
#if 0 //v: commented to remove aik signing with policykey
...
// --------------------------------------------------------------------------
#ifndef NOKNS //v: using same code for library too
...
bool server()
{
    ...
    // Signing key
    g_pSigningKey= (RSAKey*) ReadKeyfromFile_i(g_szPrivateKeyFileName);
