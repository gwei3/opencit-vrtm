...
#else
#ifdef TEST
    fprintf(stdout, "computeTPM12compositepcrDigest\n");
    PrintBytes("PCR Mask: ", g_oTpm.m_rgpcrMask, 3);
#endif
    // reconstruct PCR composite and composite hash
...
bool quotewithTPM(int inSize, byte* inData, int* poutSize, byte* outData, byte* plocality)
{
    byte    newout[1024];
    bool    fRet= g_oTpm.quoteData(inSize, inData, (unsigned*) poutSize, newout, plocality);
...
{
	return g_oTpm.getPcrSelection(rgiPCRs, pcrNum);
}
