...
#ifdef CSR_REQ
    m_pem_sealedprivateKeyValid= false;
    m_pem_sealedprivateKeySize= 0;
    m_pem_sealedprivateKey= NULL;
    m_pem_privateKeyValid= false;
    m_pem_privateKeyType= KEYTYPENONE;
    m_pem_privateKeySize= 0;
    m_pem_privateKey= NULL;
    m_pem_szPrivateKeyName= NULL;
    m_pem_szPrivateSubjectName= NULL;
    m_pem_szPrivateSubjectId= NULL;
    //m_pem_serialized_privateKeySize= 0;
    //m_pem_serialized_privateKey= NULL;
#endif
                        ...
                        int sizetoAttest, byte* toAttest, int* psizeAttested, byte* attest)
{
    ...
    char encoded_host_measurement[100];
    int len_enc=100;

    toBase64(hostedMeasurementSize, hostedMeasurement, &len_enc, encoded_host_measurement);
    fwrite(encoded_host_measurement , 1 , len_enc , stdout );

    const char* szProlog= "JLMQUOTE";
    SHA256_Update(&sha256, hostedMeasurement, hostedMeasurementSize);
    SHA256_Update(&sha256, toAttest, sizetoAttest);
    SHA256_Update(&sha256,(byte*)szProlog, strlen(szProlog));
    ...
    printf("FINALLLLLLL : %s", outputBuffer);
    FILE *fp = fopen("data-value","w");
    fprintf(fp,"%s",outputBuffer);
    fclose(fp);
  
    FILE *inFileFP, *outFileFP;
   // FILE *inFileFP, *privateKeyFP, *outFileFP;
    inFileFP = fopen("data-value", "r");
    //privateKeyFP = fopen("/tmp/privatekey", "r");
...
/*    if (!PEM_read_RSAPrivateKey(privateKeyFP, &rsa_pkey, NULL, NULL)) {
    ...
    BIO_write(bio_mem, m_pem_privateKey, m_pem_privateKeySize);

    if (!PEM_read_bio_RSAPrivateKey(bio_mem, &rsa_pkey, NULL, NULL)) {
        fprintf(stderr, "Error loading RSA Private Key File.\n");
        return 2;
    }
    ...
    //fwrite(sig, siglen1, 1, outFileFP);
    //free(sig);
    EVP_PKEY_free(pkey);

    memcpy(attest, sig, siglen);
    free(sig);
    *psizeAttested=siglen;
    remove("data-value");
...
bool taoEnvironment::initTao(u32 symType, u32 pubkeyType)
{
    ...
    #ifdef CSR_REQ
    m_pem_privateKeyValid= myInit.m_pem_privateKeyValid;

    if(myInit.m_pem_privateKeyValid) {
        m_pem_privateKeyType= myInit.m_pem_privateKeyType;
        m_pem_privateKeySize= myInit.m_pem_privateKeySize;
        m_pem_privateKey= (byte*) malloc(m_pem_privateKeySize);
        if(m_pem_privateKey==NULL) {
            return false;
        }
        memcpy(m_pem_privateKey, myInit.m_pem_privateKey, m_pem_privateKeySize);
    }
    #endif
...
#ifdef CSR_REQ
	if(!m_pem_sealedprivateKeyValid) {
            if(m_pem_privateKeyType!=KEYTYPERSA1024INTERNALSTRUCT &&
               m_pem_privateKeyType!=KEYTYPERSA2048INTERNALSTRUCT) {
                fprintf(g_logFile, "taoEnvironment::saveTao: cant serialize private key type\n");
                return false;
            }
	    if(!localsealKey(m_pem_privateKeyType, m_pem_privateKeySize,
                            m_pem_privateKey, &m_pem_sealedprivateKeySize,
                            &m_pem_sealedprivateKey)) {
                fprintf(g_logFile, "taoEnvironment::saveTao: cant seal private key\n");
                return false;
            }

        
        m_pem_sealedprivateKeyValid= true;
    	}
...
#ifdef CSR_REQ
	if(!m_fileNames.putBlobData(m_fileNames.m_pem_szprivateFile, m_pem_sealedprivateKeyValid,
                                m_pem_sealedprivateKeySize, m_pem_sealedprivateKey)) {
        	fprintf(g_logFile, "taoEnvironment::saveTao: cant save private key\n");
	        return false;
    	}
#endif
...
#ifdef CSR_REQ
	if(!m_fileNames.getBlobData(m_fileNames.m_pem_szprivateFile, &m_pem_sealedprivateKeyValid,
                                &m_pem_sealedprivateKeySize, &m_pem_sealedprivateKey)) {
        fprintf(g_logFile, "taoEnvironment::restoreTao: cant retrieve private key\n");
        return false;
        }
...
#ifdef CSR_REQ
    if(!m_pem_privateKeyValid) {
	if(!localunsealKey(m_pem_sealedprivateKeySize, m_pem_sealedprivateKey,
                           &m_pem_privateKeyType, &m_pem_privateKeySize,
                           (byte**)&m_pem_privateKey)) {
            fprintf(g_logFile, "taoEnvironment::restoreTao: cant unseal private key\n");
	    m_pem_privateKeyValid= false;
            return false;
        }
