 ...
 */
    ...
    //HL: add the null character at the end, for later buyf processing
    //m_hostCertificate[m_hostCertificateSize-1]='\0';
...
{
	#ifdef TEST
	fprintf(stdout, "HOST TYpe: %d", m_hostType);
	#endif
    switch(m_hostType) {
      default:
		asm ("int $0x3");
      ...
      case PLATFORMTYPEHW:
		*ptype = HASHTYPETPM;
	#ifdef TEST
	fprintf(stdout, "getMeasurementTPM");
	#endif
...
#ifdef TEST
    fprintf(g_logFile, "taoHostServices::Attest, size to attenst %d\n", sizetoAttest);
...
bool taoHostServices::getPcrSelection(int * rgiPCRs, int *pcrNum)
{
	switch(m_hostType) {
      default:
         return false;
      case PLATFORMTYPEHW:
		return getPcrSelectionTPM(rgiPCRs, pcrNum);
      case PLATFORMTYPELINUX:
        asm ("int $0x3"); //return quotefromDeviceDriver(sizetoAttest, toAttest, psizeAttested, attested);
    }
}
