...
#define HASHTYPEHOSTEDPROGRAM                  2
...
//taoinit state machine
#define TAOINIT_FAIL						-1
#define TAOINIT_INPROGRESS					 1
#define TAOINIT_DONE						 2
...
public:
	...
	byte 		locality() { return m_locality; }
...
#ifdef CSR_REQ
    bool                m_pem_sealedprivateKeyValid;
    int                 m_pem_sealedprivateKeySize;
    byte*               m_pem_sealedprivateKey;
    bool                m_pem_privateKeyValid;
    u32                 m_pem_privateKeyType;
    int                 m_pem_privateKeySize;
    byte*               m_pem_privateKey;
//    u32                 m_pem_serialized_privateKeyType;
 //   int                 m_pem_serialized_privateKeySize;
  //  char*               m_pem_serialized_privateKey;
    char*               m_pem_szPrivateKeyName;
    char*               m_pem_szPrivateSubjectName;
    char*               m_pem_szPrivateSubjectId;
#endif
...
public:
    ...
    int                 m_pem_privateKeySize;
    byte*               m_pem_privateKey;
//    int                 m_pem_sizeserialized_PrivateKey;
//    char*               m_pem_szserialized_PrivateKey;
#endif
    ...
    //v: added to fit in Mt Wilson model
    bool 		handleCertResponse(const char* srv_response);
    bool                handlex509CertResponse(const char* srv_response, int size);
    bool 		generateCertRequest(u32 keyType, const char* szKeyName, 
                                const char* szSubjectName, const char* szSubjectId, char *nonceStr,
                                char* msg_buf, int maxSize);

    bool                generateCSR(u32 keyType, const char* szKeyName,
                                const char* szSubjectName, const char* szSubjectId, char * nonceStr,
                                char* msg_buf, int maxSize);
                                
    int 		do_rp_certification(u32 keyType, const char* szKeyName, 
                                const char* szSubjectName, const char* szSubjectId);
};
