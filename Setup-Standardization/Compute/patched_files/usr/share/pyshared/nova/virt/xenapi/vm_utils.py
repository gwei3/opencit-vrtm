...
""" MH code in create_vm : other_config of VM is updated with is_measured flag: 'other_config': {'nova_uuid': str(instance['uuid']), 'is_measured' : is_measured} """
def create_vm(session, instance, name_label, kernel, ramdisk,
    ...
    rec = {
        ...
        'other_config': {'nova_uuid': str(instance['uuid']), 'is_measured' : is_measured},
        'PCI_bus': '',
...
def _create_kernel_image(context, session, instance, name_label, image_id,
    ...
    """
    if filename == "":
        return _fetch_image(context, session, instance, name_label,
                                 image_id, image_type)
    else:
...
def _create_cached_image(context, session, instance, name_label,
    ...
    LOG.debug(_("mhagent hook: vm_utils.py _create_cached_image"))
    ...
    """ MH function call to fetch manifest file in case of disk image is cached """
    _fetch_manifest_file(context, session, instance, image_id, vdis)
    """ MH code end"""
    return vdis

""" MH start to _fetch_manifest_file """
def _fetch_manifest_file(context, session, instance, image_id, vdis):
    LOG.debug("Inside Fetch Manifest File")
    LOG.debug(_("Asking xapi to fetch vhd image %s"), image_id,
              instance=instance)

    handler = _choose_download_handler(context, instance)
    try:
        handler.download_manifest_file(context, session, instance, image_id, vdis)
        ...
        default_handler.download_manifest_file(context, session, instance, image_id, vdis)

""" MH code end _fetch_manifest_file """ 
...
def _create_image(context, session, instance, name_label, image_id,
    ...
    # jbuhacoff log statement
    LOG.debug(_("mhagent hook: vm_utils.py _create_image"))
...
def _fetch_image(context, session, instance, name_label, image_id, image_type):
    ...
    if image_type == ImageType.DISK_VHD:
        ...
        """ MH start of _fetch_encrypted_disk_image """
        """ Check mh_encrypted and mh_checksum attributes present in instance then do the encrypted image download"""
    elif 'mh_encrypted' in instance and 'mh_checksum' in instance:
        vdis = _fetch_encrypted_disk_image(context, session, instance, name_label, image_id, image_type, instance['mh_encrypted'], instance['mh_checksum'])
        """ MH end of _fetch_encrypted_disk_image """
    else:
...
def _fetch_vhd_image(context, session, instance, image_id):
    ...
    """
    # jbuhacoff log statement
    LOG.debug(_("mhagent hook: vm_utils.py _fetch_vhd_image asking xapi to fetch image %s"), image_id)
    ...
    # jbuhacoff log statement
    LOG.debug(_("mhagent hook: vm_utils.py will download image from glance"))
...
""" MH start of _fetch_encrypted_disk_image """
""" This function will do the download of encrypted non vhd images """
def _fetch_encrypted_disk_image(context, session, instance, name_label, image_id, image_type, mh_encrypted, mh_checksum):
    ...
    try:
        ...
        args['glance_host'] = g_host
        args['glance_port'] = g_port
        args['glance_use_ssl'] = g_use_ssl
        args['sr_path'] = get_sr_path(session)
        args['mh_encrypted'] = mh_encrypted
        args['mh_checksum']  = mh_checksum
        args['offset'] = offset
        args['identity_header']= glance.generate_identity_headers(context)
        args['image_type'] = image_type
        # Now do the work of download and decryption
        # Here we are serializing args argument. It will create a string representation of args and then we are passing it to xen plugin because XEN xapi server doesn't support the python data type
        args_rec = {'params': pickle.dumps(dict(args=[], kwargs=args))}
        actual_vdi_size = session.call_plugin('glance', 'download_encrypted_nonvhd', args_rec);
        ...
        if image_type in (ImageType.KERNEL, ImageType.RAMDISK):
            ...
            args['image-size'] = str(pickle.loads(actual_vdi_size))
            if CONF.cache_images:
                args['cached-image'] = image_id
