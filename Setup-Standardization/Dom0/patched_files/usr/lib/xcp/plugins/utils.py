...
def finish_subprocess(proc, cmdline, cmd_input=None, ok_exit_codes=None):
    ...
    if ok_exit_codes is None:
        ok_exit_codes = [0]
    LOG.info("ok_exit_codes ------ %s" % ok_exit_codes)
    LOG.info("cmd_input ------ %s" % cmd_input)
...
""" MH start of decrypt: decrypts glance stream """
""" Decrypt function is using openssl for decryption not the mhagent """
def decrypt(file_obj, path ):
    LOG.info("Inside decrypt...")
    cmdline = ["/bin/sh","-c","/usr/bin/openssl enc -d -aes-128-ofb -pass pass:intelrp -out %s" % (path)]
    proc = make_subprocess(cmdline, stderr=True, stdin=True, stdout=True)
    try :
        while True :
        ...
        raise         
    finish_subprocess(proc, cmdline)
    LOG.info("end")
    
""" MH end of decrypt"""

def extract_tarball(fileobj, path, callback=None, extra_params=None):
    ...
    """
    # Changes for MH-Agent - Start
    LOG.info("jbuhacoff: starting streaming image to '%s'" % path)
    if extra_params is None: extra_params = { 'mh_encrypted': 'false' }
    LOG.info("jbuhacoff: mh_encrypted=%s" % extra_params['mh_encrypted'])
    LOG.info("jbuhacoff: mh_checksum=%s" % extra_params['mh_checksum'])
    #LOG.info("jbuhacoff: mh_dek_file=%s" % extra_params['mh_dek_file'])
    if extra_params['mh_encrypted'] == 'true':
        #LOG.info("jbuhacoff: decrypting streaming image with key at %s" % extra_params['mh_dek_file'])
        subprocess.call(['/usr/local/bin/mhagent','log','Decrypting VM image'])
    
        tar_cmd = ["/bin/sh","-c","/usr/bin/openssl enc -d -aes-128-ofb -pass pass:intelrp | tar -zx --directory=%s" % (path)]
    else:
        LOG.info("jbuhacoff: streaming plain image")
        tar_cmd = ["tar", "-zx", "--directory=%s" % path]
    # Changes for MH-Agent - End
    

    tar_proc = make_subprocess(tar_cmd, stderr=True, stdin=True, stdout=True)
    ...
    finish_subprocess(tar_proc, tar_cmd)
    # Changes for MH-Agent - Start
    LOG.info("jbuhacoff: finished streaming image to '%s'" % path)
    subprocess.call(['/usr/local/bin/mhagent','log','Decrypted VM image'])
    # Changes for MH-Agent - End
