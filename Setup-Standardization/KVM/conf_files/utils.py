...
# jbuhacoff modified function signature to insert "instance" between image_id and user_id
#def fetch_image(context, target, image_id, user_id, project_id):
def fetch_image(context, target, image_id, instance, user_id, project_id, max_size=0):
    """Grab image."""
    # MH start of mhagent hook for logging
    if 'mh_encrypted' in instance and 'mh_checksum' in instance and 'mh_dek_url' in instance:
        LOG.info(_("decrypting the image"))
        subprocess.call(['/usr/local/bin/mhagent','log','utils fetch_image 1 with info','--project-id='+project_id,'--instance-name='+instance['name'],'--base-image='+image_id,'--image-id='+image_id,'--target='+target,'--checksum='+instance['mh_checksum'],'--dek-url='+instance['mh_dek_url']])
    else:
