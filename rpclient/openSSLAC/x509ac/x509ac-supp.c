...
extern "C" 
{
	...
	X509AC_ISSUER_SERIAL* X509_get_basecertID(X509 *x)
	{
		...
		gen = (GENERAL_NAME*) X509_NAME_new();
		gen->type = GEN_DIRNAME;
		gen->d.directoryName = x->cert_info->subject;
	...
	*/
	...
	int X509AC_set_version(X509AC *a, long version)
	{
		...
		/* set how to refer to the issuer */
		if(version == 1)
		{
		...
		else if(version == 2)
		{
			...
			return(ASN1_INTEGER_set(a->info->version,2));
		}
		return(0);
	}
	...
	*/
	...
	{
		X509AC_ISSUER_SERIAL *in = NULL,*aux = NULL;
		if( (a == NULL)||(bci == NULL) )
			return(0);
		if( a->info->holder == NULL)
			return(0);
	...
	{
		X509AC_OBJECT_DIGESTINFO *in,*aux;
		if( (a == NULL)||(odig == NULL) )
			return(0);
		if( a->info->holder == NULL)
			return(0);

		if(a->info->holder->objectDigestInfo == NULL)
		{
			in = a->info->holder->objectDigestInfo;
			in = (X509AC_OBJECT_DIGESTINFO*)X509AC_OBJECT_DIGESTINFO_dup(aux);
		}
		else
		{
			X509AC_OBJECT_DIGESTINFO_free(a->info->holder->objectDigestInfo);
			in = a->info->holder->objectDigestInfo;
			in = (X509AC_OBJECT_DIGESTINFO*)X509AC_OBJECT_DIGESTINFO_dup(aux);
		}

		return(a->info->holder->objectDigestInfo != NULL);
	}
	/*
	...
	*/
	int X509AC_set_issuer_objectDigestInfo(X509AC* a, X509AC_OBJECT_DIGESTINFO *odig)
	{
		...
		if(a->info->issuer->d.v2Form->digest == NULL)
		{
			in = a->info->issuer->d.v2Form->digest;
			in = (X509AC_OBJECT_DIGESTINFO *) X509AC_OBJECT_DIGESTINFO_dup(aux);
		}
		else
		{
			X509AC_OBJECT_DIGESTINFO_free(a->info->issuer->d.v2Form->digest);
			a->info->issuer->d.v2Form->digest = (X509AC_OBJECT_DIGESTINFO *) X509AC_OBJECT_DIGESTINFO_dup(aux);
		}

		return(a->info->issuer->d.v2Form->digest != NULL);
	}
	...
	*/
	...
	int X509AC_add_attribute(X509AC *a, X509_ATTRIBUTE *attr)
	{
        ...
        if( (a == NULL)||(attr==NULL) )
			return(0);
		sk = &a->info->attributes;
		//sk_X509_ATTRIBUTE_push(a->info->attributes, X509_ATTRIBUTE_dup(attr));
		return(/*X509at_add1_attr(sk,attr)*/X509AC_add_X509_ATTRIBUTE(a,attr));
	}
...
new_attrib:
	...
	{
		X509_ATTRIBUTE *attr_aux, *new_attr;
		STACK_OF(X509_ATTRIBUTE) *psk = NULL;
		STACK_OF(X509_ATTRIBUTE) **ppsk = NULL;
		int i = 0;
...
new_attrib:
	...
	*/
	int X509AC_verify_cert(X509_STORE_CTX * verify_ctx, X509AC * ac) {
		...
		diff = X509_cmp_current_time(ac->info->validity->notBefore);
		if(diff == 0) {
			fprintf(stderr,"Error in not-before field\n");
			return -1;
		}
		if(diff > 0) {
			fprintf(stderr,"Attribute certificate not yet valid\n");
			return -1;
		}

		diff = X509_cmp_current_time(ac->info->validity->notAfter);
		if(diff == 0) {
			fprintf(stderr,"Error in not-after field \n");
			return -1;
		}
		if(diff < 0) {
			fprintf(stderr, "Attribute certificate expired \n");
			return -1;
		}
		...
		if (!acIssuer) {
			fprintf(stderr,"Error getting AC issuer name \n");
			return -1;
		}
		...
		for (i = 0; i < sk_X509_num(verify_ctx->untrusted); i++)
		{
			...
			if (!subject) {
				fprintf(stderr,"Error getting cert subject name, AAAAAHHH\n");
				return -1;
			}
		...
		if (pkey == NULL) {
			fprintf(stderr,"Error getting pub key from issuer cert\n");
			return(-1);
		}
