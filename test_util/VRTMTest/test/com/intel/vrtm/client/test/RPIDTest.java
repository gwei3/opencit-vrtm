...
public class RPIDTest {
	...
	public static void main(String[] args) throws IOException{
		...
		TCBuffer tcBuffer = Factory.newTCBuffer( RPCCall.GET_RPID);
		
		// first replace the %s of xmlRPCBlob by VMUUID, rpcore accept all method input arguments in base64 format
		
		String base64InputArgument = String.format(xmlRPCBlob, DatatypeConverter.printBase64Binary(args[2].getBytes()));
		System.out.println(base64InputArgument);
		tcBuffer.setRPCPayload(base64InputArgument.getBytes());
		tcBuffer.serializeTCBuffer(System.out);
		
		// create instance of RPClient, One instance for App life time is sufficient 
		// to do processing 
